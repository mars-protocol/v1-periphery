...
async function setupTest() {
    ...
    console.log(chalk.green(`$MARS deployed successfully, address : ${chalk.cyan(mars_token_address)}`));
    ...
    var contract_mars_balance_before_transfer = await getCW20Balance(terra, mars_token_address, airdrop_contract_address);
    var deployer_mars_balance_before_transfer = await getCW20Balance(terra, mars_token_address, deployer.key.accAddress);

    await transferCW20Tokens(terra, deployer, mars_token_address, airdrop_contract_address, 2500000 * 10**6 );

    var contract_mars_balance_after_transfer = await getCW20Balance(terra, mars_token_address, airdrop_contract_address);
...
async function testUpdateConfig(terra_merkle_roots: [string], evm_merkle_roots: [string]) {
    ...
    const airdropConfigResponse = await getAirdropConfig(terra, airdrop_contract_address);
    expect(airdropConfigResponse).to.deep.equal({ mars_token_address: mars_token_address,
                                                  owner: deployer.key.accAddress,
                                                  terra_merkle_roots: terra_merkle_roots,
                                                  evm_merkle_roots: evm_merkle_roots,
                                                  from_timestamp: init_timestamp,
...
async function testClaimByTerraUser(claimeeWallet:Wallet, amountClaimed:number, merkle_proof: any, root_index: number ) {
   ...
   expect( is_claimed_before ).to.deep.equal( { is_claimed: false } );
    ...
    var user_mars_balance_before_claim = await getCW20Balance(terra, mars_token_address, claimeeWallet.key.accAddress);

    await claimAirdropForTerraUser(terra,claimeeWallet, airdrop_contract_address,amountClaimed,merkle_proof,root_index);

    var contract_mars_balance_after_claim = await getCW20Balance(terra, mars_token_address, airdrop_contract_address);
    var user_mars_balance_after_claim = await getCW20Balance(terra, mars_token_address, claimeeWallet.key.accAddress);
    ...
    expect(Number(contract_mars_balance_before_claim) - Number(contract_mars_balance_after_claim)).to.equal(amountClaimed);
    expect(Number(user_mars_balance_after_claim) - Number(user_mars_balance_before_claim)).to.equal(amountClaimed);
...
async function testClaimByEvmUser(recepientWallet:Wallet, evm_address:string, amountClaimed:number, public_key:string, signed_msg_hash:string, signature:string, merkle_proof: any, root_index: number ) {
    ...
    var contract_mars_balance_after_claim = await getCW20Balance(terra, mars_token_address, airdrop_contract_address);
    var recepientWallet_mars_balance_after_claim = await getCW20Balance(terra, mars_token_address, recepientWallet.key.accAddress);

    let is_claimed_after = await isAirdropClaimed(terra, airdrop_contract_address, evm_address.replace('0x','').toLowerCase() );
    expect( is_claimed_after ).to.deep.equal( { is_claimed: true } ); 

    expect(Number(contract_mars_balance_before_claim) - Number(contract_mars_balance_after_claim)).to.equal(amountClaimed);
    expect(Number(recepientWallet_mars_balance_after_claim) - Number(recepient_mars_balance_before_claim)).to.equal(amountClaimed);
...
//----------------------------------------------------------------------------------------
// (ADMIN FUNCTION) Transfer MARS Tokens : Test
//----------------------------------------------------------------------------------------

async function testTransferMarsByAdmin(recepient_address:string, amountToTransfer:number) {
    process.stdout.write("Should transfer MARS from the Airdrop Contract correctly... ");
    
    var contract_mars_balance_before_claim = await getCW20Balance(terra, mars_token_address, airdrop_contract_address);
    var recepient_mars_balance_before_claim = await getCW20Balance(terra, mars_token_address, recepient_address );

    await transferMarsByAdminFromAirdropContract(terra,deployer, airdrop_contract_address, recepient_address, amountToTransfer);

    var contract_mars_balance_after_claim = await getCW20Balance(terra, mars_token_address, airdrop_contract_address);
    var recepientWallet_mars_balance_after_claim = await getCW20Balance(terra, mars_token_address, recepient_address );

    expect(Number(contract_mars_balance_before_claim) - Number(contract_mars_balance_after_claim)).to.equal(amountToTransfer);
    expect(Number(recepientWallet_mars_balance_after_claim) - Number(recepient_mars_balance_before_claim)).to.equal(amountToTransfer);

    console.log(chalk.green("\nTransfer of MARS tokens by the deployer with admin privileges processed successfully"));                                
}

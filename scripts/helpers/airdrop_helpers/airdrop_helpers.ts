...
export async function updateAirdropConfig(
  ...
  wallet: Wallet,
  airdropContractAdr: string,
  new_config: any
) {
  let resp = await executeContract(
    terra,
    wallet,
    airdropContractAdr,
    new_config
...
export async function transferAstroByAdminFromAirdropContract(
  terra: LocalTerra | LCDClient,
  wallet: Wallet,
  airdropContractAdr: string,
  recipient: string,
  ...
  try {
    let transfer_astro_msg = {
      transfer_astro_tokens: {
        recipient: recipient,
...
export async function getAirdropConfig(
  ...
  try {
    let res = await terra.wasm.contractQuery(airdropContractAdr, {
      config: {},
    });
    return res;
  } catch {

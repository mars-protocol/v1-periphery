...
import airdropdataTerra from "./airdrop_data/Terra_airdrop_recipients.json";
import airdropdataEvm from "./airdrop_data/EVM_airdrop_recipients.json";
...
export async function getMerkleRootsForEVMUsers() {
  ...
  for (let i = 0; i < n; i++) {
    let evm_data = prepareDataForMerkleTree(
      airdropdataEvm.data,
      i * Math.round(airdropdataEvm.data.length / n),
      (i + 1) * Math.round(airdropdataEvm.data.length / n)
    );
...
export function get_EVM_MerkleProof(leaf: { address: string; amount: string }) {
  ...
  for (let i = 0; i < n; i++) {
    let evm = prepareDataForMerkleTree(
      airdropdataEvm.data,
      i * Math.round(airdropdataEvm.data.length / n),
      (i + 1) * Math.round(airdropdataEvm.data.length / n)
    );

...
fn proper_initialization() {
    ...
    let msg = InstantiateMsg {
        ...
        mars_token: "reward0000".to_string(),
...
#[test]
fn test_unbond_without_reward_claim() {
    let mut deps = mock_dependencies(&[]);
...
#[test]
fn test_unbond_with_reward_claim() {
    let mut deps = mock_dependencies(&[]);
    ...
    assert_eq!(
        from_binary::<StakerInfoResponse>(
            &query(
                ...
                QueryMsg::StakerInfo {
                    staker: "addr0000".to_string(),
                    timestamp: Some(mock_env().block.time.plus_seconds(120).seconds()),
                },
            )
            .unwrap()
        )

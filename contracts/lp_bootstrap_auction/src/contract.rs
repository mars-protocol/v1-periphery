...
use mars_periphery::helpers::{zero_address, build_approve_cw20_msg, build_send_native_asset_msg, cw20_get_balance, option_string_to_addr, build_transfer_cw20_token_msg, get_denom_amount_from_coins } ;
use mars_periphery::lp_bootstrap_auction::{Cw20HookMsg, UserInfoResponse, StateResponse, ConfigResponse, WithdrawalStatus, ExecuteMsg, InstantiateMsg,UpdateConfigMsg, QueryMsg , CallbackMsg } ;
...
pub fn instantiate( deps: DepsMut, _env: Env, _info: MessageInfo, msg: InstantiateMsg) -> StdResult<Response> {
    ...
    let config = Config {
        ...
        mars_lp_pool:  option_string_to_addr(deps.api, msg.mars_lp_pool, zero_address())?, 
...
pub fn execute( deps: DepsMut, env: Env, info: MessageInfo, msg: ExecuteMsg)  -> Result<Response, StdError> {
    match msg {
        ...
        ExecuteMsg::AddLiquidityToMarsPool { slippage }  => handle_add_liquidity_to_mars_pool(deps, env,  info, slippage),
...
pub fn receive_cw20(deps: DepsMut, env: Env, info: MessageInfo, cw20_msg: Cw20ReceiveMsg) -> Result<Response, StdError> {
    ...
    match from_binary(&cw20_msg.msg)? {
        Cw20HookMsg::DelegateMarsTokens { user_address } => handle_delegate_mars_tokens(deps, env, info,  user_address, amount.into())                   
    }
}
...
fn _handle_callback(
    ...
    match msg {
        ...
        CallbackMsg::UpdateStateOnRewardClaim {
            user_address,
            prev_mars_balance
        } => update_state_on_reward_claim(deps, env, user_address, prev_mars_balance)
    }
}
...
pub fn handle_update_config( deps: DepsMut, info: MessageInfo, new_config: UpdateConfigMsg ) -> StdResult<Response> { 
    ...
    config.mars_lp_pool = option_string_to_addr(deps.api, new_config.mars_lp_pool, config.mars_lp_pool)?;
    ...
    config.mars_rewards = new_config.mars_rewards.unwrap_or(config.mars_rewards);
...
/// @dev Delegates MARS tokens to be used for the LP Bootstrapping via auction. Callable only by Airdrop / Lockdrop contracts
/// @param user_address : User address who is delegating the MARS tokens for LP Pool bootstrap via auction
/// @param amount : Number of MARS Tokens being delegated
pub fn handle_delegate_mars_tokens( 
    deps: DepsMut, 
    _env: Env, 
    info: MessageInfo,
    user_address: String,  
    ...
    user_info.mars_delegated += amount;
    ...
    Ok(Response::new()        
    .add_attributes(vec![
        attr("action", "Auction::ExecuteMsg::DelegateMarsTokens"),
        attr("user", user_address.to_string() ),
        attr("mars_delegated", amount)
    ]))
}
...
/// @dev Admin Function to allow users to delegate their MARS Tokens to the LP Bootstrap auction contract
/// @param amount_to_delegate Amount of MARS to be delegate
pub fn handle_add_liquidity_to_mars_pool( 
    deps: DepsMut, 
    _env: Env, 
    info: MessageInfo,
    ...
    let approve_mars_msg = build_approve_cw20_msg( config.mars_token_address.to_string(), config.mars_lp_pool.to_string(), state.total_mars_deposited.into() )?;
    ...
    .add_messages(msgs_)        
    .add_attributes(vec![
        attr("action", "Auction::ExecuteMsg::AddLiquidityToMarsPool"),
        attr("mars_deposited", state.total_mars_deposited ),
...
/// @dev Facilitates MARS Reward claim for users
pub fn handle_claim_rewards(  deps: DepsMut, _env: Env,  info: MessageInfo) -> Result<Response, StdError> {
    ...
    if user_info.mars_delegated == Uint256::zero() && user_info.ust_deposited == Uint256::zero() {
        return Err(StdError::generic_err("No rewards to claim"));
    }

    let mut cosmos_msgs = vec![];

    // QUERY :: ARE MARS REWARDS TO BE CLAIMED FOR LP STAKING > 0 ?
    ...
    if state.are_staked {
        ...
        if unclaimed_rewards_response.pending > Uint128::zero() {
            cosmos_msgs.push(build_claim_mars_rewards(_env.contract.address.clone(), config.lp_token_address,config.lp_staking_contract.clone())?);
        }        
    }

    // QUERY :: Current MARS Contract Balance
    // -->add CallbackMsg::UpdateStateOnRewardClaim{} msg to the cosmos msg array    
    let mars_balance: cw20::BalanceResponse = deps.querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
    ...
    let update_state_msg = CallbackMsg::UpdateStateOnRewardClaim {  user_address: depositor_address.clone(), 
                                                                                prev_mars_balance: mars_balance.balance.into()
                                                                            }.to_cosmos_msg(&_env.contract.address)?;
...
pub fn handle_withdraw_unlocked_lp_shares( 
    ...
    if state.are_staked {
        ...
        if unclaimed_rewards_response.pending > Uint128::zero() {
            cosmos_msgs.push(build_claim_mars_rewards(_env.contract.address.clone(), config.lp_token_address.clone(), config.lp_staking_contract.clone())?);
        }        
    }

    // QUERY :: Current MARS Token Balance
    // -->add CallbackMsg::UpdateStateOnRewardClaim{} msg to the cosmos msg array    
    let mars_balance: cw20::BalanceResponse = deps.querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
                                                                    contract_addr: config.mars_token_address.to_string(),
                                                                    msg: to_binary(&cw20_base::msg::QueryMsg::Balance {
...
pub fn update_state_on_reward_claim( deps: DepsMut, _env: Env, user_address:Addr, prev_mars_balance: Uint256) -> StdResult<Response> {
    let config = CONFIG.load(deps.storage)?;
    let mut state = STATE.load(deps.storage)?;
    let mut user_info =  USERS.may_load(deps.storage, &user_address.clone() )?.unwrap_or_default();

    // QUERY CURRENT LP TOKEN BALANCE :: NEWLY MINTED LP TOKENS
    let cur_mars_balance = cw20_get_balance(&deps.querier, config.mars_token_address.clone(), _env.contract.address )?;
    let mars_claimed = Uint256::from(cur_mars_balance) - prev_mars_balance;

    let mut user_mars_rewards = Uint256::zero();
    ...
    if user_info.total_auction_incentives == Uint256::zero() {
        user_info.total_auction_incentives = calculate_auction_reward_for_user(&state, &mut user_info, config.mars_rewards);
    }

    user_mars_rewards += calculate_claimable_auction_reward_for_user(_env.block.time.seconds(), &config.clone(), &user_info) ;
    user_info.claimed_auction_incentives += user_mars_rewards; 

    if mars_claimed > Uint256::zero() {
        update_mars_rewards_index(&mut state, mars_claimed);
        staking_reward = compute_user_accrued_reward(&state, &mut user_info);
        user_mars_rewards += staking_reward;
    }

    // CHECK :: MARS Rewards must be > 0
    ...
    let transfer_mars_rewards =  build_transfer_cw20_token_msg(user_address.clone(), config.mars_token_address.clone().to_string(), user_mars_rewards.into())?;

    Ok(Response::new()
    .add_message(transfer_mars_rewards)
    .add_attributes(vec![
        ("action", "Auction::CallbackMsg::UpdateStateOnRewardClaim"),
        ("user_address", user_address.to_string().as_str()),
        ("auction_participation_reward", &(user_mars_rewards - staking_reward).to_string() ),
...
fn query_user_info(deps: Deps, _env:Env, user_address: String) -> StdResult<UserInfoResponse> {
    ...
    if state.are_staked {
        ...
        if unclaimed_rewards_response.pending > Uint128::zero() {
            update_mars_rewards_index(&mut state, unclaimed_rewards_response.pending.into());
            claimable_staking_reward = compute_user_accrued_reward(&state, &mut user_info);    
        }
    }

    Ok(UserInfoResponse { 
        mars_delegated: user_info.mars_delegated,
...
/// Calculates MARS rewards for participation in the auction for a user
fn calculate_auction_reward_for_user(state: &State, user_info: &UserInfo, mars_rewards_alloc: Uint256 ) -> Uint256 {

    // In-case MARS incentives for participation in the auction are already claimed or total MARS delegated / UST deposited is currently 0
    if user_info.total_auction_incentives > Uint256::zero() || state.total_mars_deposited == Uint256::zero() || state.total_ust_deposited == Uint256::zero() {
        return Uint256::zero();
    }

    let mars_rewards_alloc_half = mars_rewards_alloc.div(Decimal256::from_ratio(2u32, 1u32));
    let mut total_mars_rewards  = Uint256::zero();

    // Calculate rewards for MARS Allocation by user
    if user_info.mars_delegated > Uint256::zero() {
        total_mars_rewards +=  mars_rewards_alloc_half * Decimal256::from_ratio(user_info.mars_delegated, state.total_mars_deposited);
    }
    // Calculate rewards for UST provided by user
    if user_info.ust_deposited > Uint256::zero() {
        total_mars_rewards +=  mars_rewards_alloc_half * Decimal256::from_ratio(user_info.ust_deposited, state.total_ust_deposited );
    }
    total_mars_rewards
}
...
// Accrue MARS rewards by updating the reward index
fn update_mars_rewards_index(state: &mut State, mars_accured: Uint256)  {
    if !state.are_staked {
        return;
    }
    let mars_rewards_index_increment = Decimal256::from_ratio( Uint256::from(mars_accured), Uint256::from(state.lp_shares_minted),);
    state.global_reward_index = state.global_reward_index + mars_rewards_index_increment;
}
...
/// Returns MARS auction incentives that a user can withdraw based on a vesting schedule
...
fn build_provide_liquidity_to_lp_pool_msg( deps: Deps, config: &Config, state: &State, slippage_tolerance_: Option<Decimal>) -> StdResult<CosmosMsg> {
    ...
    let mars_asset =  Asset { 
        info: AssetInfo::Token { contract_addr: deps.api.addr_validate(&config.mars_token_address.to_string())? },
    ...
    Ok(CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: config.mars_lp_pool.to_string(),
...
fn build_claim_mars_rewards(recepient_address:Addr, lp_token_contract: Addr , generator_contract: Addr) -> StdResult<CosmosMsg> {
    Ok(CosmosMsg::Wasm(WasmMsg::Execute {

...
pub fn instantiate(
    ...
    let config = Config {
        ...
        mars_vesting_schedule: msg.mars_vesting_schedule,
...
pub fn receive_cw20(
    ...
    match from_binary(&cw20_msg.msg)? {
        Cw20HookMsg::DelegateMarsTokens { user_address } => {
...
pub fn handle_add_liquidity_to_astroport_pool(
    ...
    let approve_astro_msg = build_approve_cw20_msg(
        ...
        state.total_mars_delegated.into(),
    )?;
    ...
    Ok(Response::new().add_messages(msgs_).add_attributes(vec![
        ...
        attr("astro_deposited", state.total_mars_delegated),
...
pub fn handle_claim_rewards(
    ...
    if state.are_staked {
        ...
        if unclaimed_rewards_response > Uint128::zero() {
            cosmos_msgs.push(build_claim_mars_rewards(
                _env.contract.address.clone(),
                config.lp_token_address,
    ...
    let update_state_msg = CallbackMsg::UpdateStateOnRewardClaim {
        ...
        prev_mars_balance: astro_balance.into(),
    }
    .to_cosmos_msg(&_env.contract.address)?;
...
pub fn update_state_on_reward_claim(
    ...
    let astro_claimed = Uint256::from(cur_astro_balance) - prev_mars_balance;
    ...
    user_mars_rewards += calculate_claimable_auction_reward_for_user(
    ...
    // MARS Generator (Staking) rewards :: Calculate the astro amount (from LP staking incentives) which can be claimed by the user
    if astro_claimed > Uint256::zero() {
        update_mars_rewards_index(&mut state, astro_claimed);
    ...
    if user_mars_rewards > Uint256::zero() {
        let transfer_mars_rewards = build_transfer_cw20_token_msg(
            user_address.clone(),
            config.mars_token_address.to_string(),
            user_mars_rewards.into(),
        )?;
        cosmos_msgs.push(transfer_mars_rewards);
    }
    ...
    Ok(Response::new()
        ...
        .add_attributes(vec![
            ...
            (
                ...
                &(user_mars_rewards - staking_reward).to_string(),
            ),
...
fn calculate_auction_reward_for_user(
    ...
    if user_info.total_auction_incentives > Uint256::zero()
        || state.total_mars_delegated == Uint256::zero()
    ...
    if user_info.mars_delegated > Uint256::zero() {
        total_mars_rewards += mars_rewards_alloc_half
            * Decimal256::from_ratio(user_info.mars_delegated, state.total_mars_delegated);
    }
...
fn update_mars_rewards_index(state: &mut State, astro_accured: Uint256) {
    if !state.are_staked {
        return;
    }
    let mars_rewards_index_increment =
        Decimal256::from_ratio(astro_accured, state.lp_shares_minted);
    state.global_reward_index += mars_rewards_index_increment;
}
...
pub fn calculate_claimable_auction_reward_for_user(
    ...
    let time_elapsed = cur_timestamp - state.pool_init_timestamp;
    if time_elapsed >= config.mars_vesting_schedule {
    ...
    let withdrawable_auction_incentives = user_info.total_auction_incentives
        * Decimal256::from_ratio(time_elapsed, config.mars_vesting_schedule);
...
fn build_provide_liquidity_to_lp_pool_msg(
    ...
    let astro_asset = Asset {
        ...
        amount: state.total_mars_delegated.into(),
    };
    let ust_asset = Asset {
...
//             mars_token_address: Some("astro_token_contract".to_string()),
...
//         assert_eq!("astro_token_contract".to_string(), value.mars_token_address);
...
//             mars_token_address:  Some("new_astro_token".to_string()),
...
//         assert_eq!(new_config.mars_token_address, Addr::unchecked("new_astro_token"));

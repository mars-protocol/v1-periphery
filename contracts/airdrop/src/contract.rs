...
use mars_periphery::airdrop::{
    UserInfoResponse, StateResponse, ConfigResponse, ClaimResponse, SignatureResponse, ExecuteMsg, InstantiateMsg, QueryMsg
};
...
pub fn handle_terra_user_claim( 
    ...
    // TRANSFER MARS IF CLAIMS ARE ALLOWED (i.e LP Boostrap auction has concluded)
    if config.are_claims_allowed {
        user_info.tokens_claimed = user_info.airdrop_amount - user_info.tokens_used_for_auction; 
        messages_.push( build_send_cw20_token_msg(user_account.clone(), config.mars_token_address.to_string(), user_info.tokens_claimed.into())? );
    }
...
pub fn handle_evm_user_claim( 
    ...
    if config.are_claims_allowed {
        user_info.tokens_claimed = user_info.airdrop_amount - user_info.tokens_used_for_auction; 
        messages_.push( build_send_cw20_token_msg(recepient_account.clone(), config.mars_token_address.to_string(), user_info.tokens_claimed.into())? );
    }
...
pub fn handle_delegate_mars_to_bootstrap_auction( 
    ...
    .add_attributes(vec![
        attr("action", "Airdrop::ExecuteMsg::DelegateMarsToBootstrapAuction"),
...
pub fn handle_withdraw_airdrop_rewards( 
    ...
    // TRANSFER MARS IF CLAIMS ARE ALLOWED (i.e LP Boostrap auction has concluded)
    user_info.tokens_claimed = user_info.airdrop_amount - user_info.tokens_used_for_auction;
    let transfer_msg = build_send_cw20_token_msg(info.sender.clone(), config.mars_token_address.to_string(), user_info.tokens_claimed.into())? ;    

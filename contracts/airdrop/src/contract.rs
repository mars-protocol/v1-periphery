...
use crate::state::{Config, CLAIMEES, CONFIG};
...
pub fn instantiate(
    ...
    let config = Config {
        ...
        mars_token_address: deps.api.addr_validate(
            &msg.mars_token_address
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::TransferMarsTokens { recepient, amount } => {
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::IsClaimed { address } => to_binary(&check_user_claimed(deps, address)?),
...
pub fn handle_update_config(
    ...
    config.mars_token_address = option_string_to_addr(
        deps.api,
...
pub fn handle_terra_user_claim(
    ...
    let transfer_msg = build_send_cw20_token_msg(
        user_account.clone(),
...
mod tests {
    ...
    use mars_periphery::airdrop::ExecuteMsg::{
        ClaimByEvmUser, ClaimByTerraUser, TransferMarsTokens, UpdateConfig,
    };
    use mars_periphery::airdrop::{ConfigResponse, InstantiateMsg, QueryMsg};
    ...
    fn test_update_config() {
        ...
        assert_eq!(
            new_config.terra_merkle_roots,
            vec!["new_terra_merkle_roots".to_string()]
        );
        assert_eq!(
            new_config.evm_merkle_roots,
            vec!["new_evm_merkle_roots".to_string()]
        );
        assert_eq!(new_config.owner, Addr::unchecked("new_owner"));
        assert_eq!(
            new_config.mars_token_address,
    ...
    fn test_claim_by_terra_user() {
        ...
        claim_f = execute(
            ...
            env.clone(),
            user_info_1.clone(),
            claim_msg_incorrect_proof.clone(),
        );
        ...
        claim_f = execute(
            ...
            env.clone(),
            user_info_1.clone(),
            claim_msg_wrong_amount.clone(),
        );
        assert_generic_error_message(claim_f, "Incorrect Merkle Proof");

        let mut is_claimed = check_user_claimed(
            deps.as_ref(),
            "terra17lmam6zguazs5q5u6z5mmx76uj63gldnse2pdp".to_string(),
        )
        .unwrap();
        ...
        is_claimed = check_user_claimed(
            deps.as_ref(),
            "terra1757tkx08n0cqrw7p86ny9lnxsqeth0wgp0em95".to_string(),
        )
        .unwrap();
        assert_eq!(is_claimed.is_claimed, false);
    ...
    fn test_claim_by_evm_user() {
        ...
        let base_config = InstantiateMsg {
            ...
            mars_token_address: Some("mars_token_contract".to_string()),
            terra_merkle_roots: Some(vec!["".to_string()]),
            evm_merkle_roots: Some(vec![
                "1680ce46cb2c916f103afb54006b53dc751edccb8c0ba668fe1311ee7592c232".to_string(),
            ]),
            from_timestamp: Some(1_575_797),
            till_timestamp: Some(1_771_797),
        };
        ...
        claim_f = execute(
            ...
            env.clone(),
            user_info_recepient_.clone(),
            claim_msg_incorrect_msg_hash.clone(),
        );
        ...
        claim_f = execute(
            ...
            env.clone(),
            user_info_recepient_.clone(),
            claim_msg_incorrect_signature.clone(),
        );
        assert_generic_error_message(claim_f, "Invalid Signature");

        let signature_response = verify_signature(
            deps.as_ref(),
            user_info_evm_address.to_string(),
            user_info_signature.to_string(),
            user_info_signed_msg_hash.to_string(),
        )
        .unwrap();
        assert_eq!(signature_response.is_valid, true);
        assert_eq!(
            signature_response.recovered_address,
            user_info_evm_address.to_string()
        );

        let mut is_claimed =
            check_user_claimed(deps.as_ref(), user_info_evm_address.to_string()).unwrap();
        assert_eq!(is_claimed.is_claimed, false);
